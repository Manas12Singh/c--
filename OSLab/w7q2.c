...
void lru(int n, int *pages, int memorySize)
{
    Queue memory;
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            for (j = (memory.front + j) % memory.capacity; j != memory.rear; j = (j + 1) % memory.capacity)
            {
                int temp = memory.arr[j];
                memory.arr[j] = memory.arr[(j + 1) % memory.capacity];
                memory.arr[(j + 1) % memory.capacity] = temp;
            }
        }
    }
    freeQueue(&memory);
